...
**本模板基於 PARA 系統**（Projects, Areas, Resources, Archives）設計，並加入 `Inbox`（輸入緩衝）與 `Identity/System`（AI 情境與治理層）來支援 AI 協作。
...
本 repo 同時支援 Claude 與 Codex 的快速理解：

- **Claude 入口**：`.claude/CLAUDE.md`（全域規則）
- **Codex 入口**：`AGENTS.md`（同一套規則的 agent 導覽）
